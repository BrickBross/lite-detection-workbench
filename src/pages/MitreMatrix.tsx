...
export default function MitreMatrix() {
  ...
  const rows = useMemo(() => {
    const filtered = MITRE_TECHNIQUES.filter((t) => (`${t.technique} ${t.tactic} ${t.name}`).toLowerCase().includes(q.toLowerCase()))
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-400">Starter technique list + coverage indicator. Extend in <code className="text-zinc-200">src/lib/mitre.ts</code>.</p>
        </div>
        <input
          value={q}
          onChange={(e) => setQ(e.target.value)}
          placeholder="Search technique, tactic, name…"
          className="w-full rounded-2xl border border-zinc-800 bg-zinc-950 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-zinc-600 md:w-80"
        />
      </div>
      ...
      <div className="rounded-3xl border border-zinc-800 bg-zinc-950 p-4">
        ...
        <div className="space-y-2">
          {rows.map((r) => (
            <div key={r.technique} className="grid grid-cols-12 items-center gap-2 rounded-2xl border border-zinc-800 bg-zinc-900/20 px-2 py-2">
              ...
              <div className="col-span-2 flex items-center justify-end gap-2">
                <span className={r.count > 0 ? 'text-xs text-zinc-200' : 'text-xs text-zinc-500'}>{r.count > 0 ? `${r.count} obj` : 'gap'}</span>
                <span className={['inline-block h-2 w-2 rounded-full', r.count > 0 ? 'bg-zinc-200' : 'bg-zinc-700'].join(' ')} />
              </div>
            </div>
          ))}
