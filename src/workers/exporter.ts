...
function mdObjective(o: Objective) {
  ...
  const refs = (o.externalReferences ?? []).filter(Boolean)
  const refsLine = refs.length ? `**External references:** ${refs.join(', ')}  \n` : ''
  return `# ${o.id}: ${o.name}
...
${telemetryLines}${otherLines}${telemetryNotesLine}${refsLine}
...
function mdObjectivePack({
  ...
  const refsLines = (objective.externalReferences ?? []).length
    ? ['## External references', (objective.externalReferences ?? []).filter(Boolean).map((x) => `- ${x}`).join('\n'), '']
    : []
  ...
  return [
    ...
    '',
    `## Response`,
    objective.responsePlan ?? '_Not provided_',
    '',
    ...refsLines,
